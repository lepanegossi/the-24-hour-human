...
# ── how the OECD averages, stated with numbers from the extract ───────────────
# Every "per day" figure is a population average over all seven days: the total is
# divided by the whole adult population, people in work and out of work alike, and
# by every date on the calendar. That is why paid work reads around three to six
# hours and not the length of a shift, and it is the one thing a reader has to know
# before any of these numbers make sense.
_paw = sorted(cp, key=lambda r: r["paw_h"])
PAW_LO, PAW_HI = _paw[0], _paw[-1]
PAW_NOTE = (f'<b>{PAW_LO["country"]}</b>&rsquo;s <b>{_hm(PAW_LO["paw_h"])}</b> is a whole '
            f'country&rsquo;s paid hours spread across every adult and every date, and '
            f'<b>{PAW_HI["country"]}</b>&rsquo;s <b>{_hm(PAW_HI["paw_h"])}</b> at the top of the '
            f'range is the same calculation, not a longer shift')

# OECD flags observations whose definition departs from the standard one. It sits in
# the raw extract, never reached the processed table, and it is the reason a gap of a
# few minutes between two countries should not be read as real. Counted, not typed —
# and the whole bullet disappears if the raw file is not next to the builder.
def _flagged_countries():
    raw = ROOT.parent / "data" / "raw" / "oecd_time_use.csv"
    if not raw.exists():
        return 0
    return len({r["REF_AREA"] for r in csv.DictReader(open(raw))
                if r.get("OBS_STATUS") == "D"})

_flagged = _flagged_countries()
DEF_NOTE = (f'<li><b>{_flagged} of the {len(cp)} countries</b> carry OECD&rsquo;s '
            f'&ldquo;definition differs&rdquo; flag on at least one category, so a gap of a few '
            f'minutes between two countries is not necessarily a real one.</li>' if _flagged else '')

# ══════════════════════════════════════════════════════════════════════════════
# credits: the stack, and the people
# ══════════════════════════════════════════════════════════════════════════════
# Brand marks are inlined as SVG path data (from simple-icons, CC0) instead of
# being fetched from an icon CDN: the block renders with no network and nothing
# reflows while it loads. Two colours per tool because half of these marks are
# near-black or near-white and the page flips from daylight to deep night.
ICON_PATHS = {
    ...
    "pandas": "M16.922 0h2.623v18.104h-2.623zm-4.126 12.94h2.623v2.57h-2.623zm0-7.037h2.623v5.446h-2.623zm0 11.197h2.623v5.446h-2.623zM4.456 5.896h2.622V24H4.455zm4.213 2.559h2.623v2.57H8.67zm0 4.151h2.623v5.447H8.67zm0-11.187h2.623v5.446H8.67Z",
...
TOOLS = [
    ...
    ("pandas",     "pandas",        "data pipeline",    "#3c2a72", "#cbb7f0"),
    ("html5",      "HTML5",         "hand-written",     "#d1441d", "#ff8a6b"),
    ("css3",       "CSS3",          "layout & theming", "#1572b6", "#6bb4e8"),
    ("javascript", "JavaScript",    "no framework",     "#a88700", "#f7df1e"),
    ("chartjs",    "Chart.js",      "rings & bars",     "#d63a5c", "#ff8fa6"),
...
TOOLROW_HTML, PEOPLE_HTML = tools_html()
...
/* the last chapter's tail padding stacked on the footer's own 76px margin and put
   186px of sky before the sources; the 110px stays on the block after <main>,
   where it is the page's bottom breathing room and not a gap between two things */
main>.wrap.tail{padding-bottom:36px}
section{margin-top:112px}
/* The hero is a full viewport tall and centres its content, so it already ends in
   a stretch of empty sky; adding the full 112px rhythm on top of that read as a
   dead gap before the first chapter. The rhythm between chapters is untouched. */
...
/* The band's own padding sits on top of the 112px chapter rhythm, so the empty sky
   before a band's heading measured 192px and after it 196px, against 112px
   everywhere else. Split the same 112px between the margin and the padding, and
   take the following chapter's margin down to match, so a band costs no more
   vertical space than a plain chapter. */
.act{position:relative;z-index:2;padding:48px 0 48px;margin-top:64px}
.act+.wrap>section:first-child{margin-top:64px}
...
.heroclock{width:176px;height:176px;margin:0 auto 20px;border-radius:50%;flex:0 0 auto;
...
@media(max-height:930px){
 ...
 .heroclock{width:152px;height:152px;margin-bottom:16px}
...
/* The clock steps down with the screen instead of jumping straight to small. The
   budget at each step is the empty space left under the scroll cue: the hero
   centres its content, so every pixel the clock grows eats half a pixel of the
   margin above and half of the one below. Measured at each breakpoint, not
   guessed -- at 895px of viewport there were 103px going spare, which is why the
   clock had no business being 84px there. */
@media(max-height:860px){.heroclock{width:140px;height:140px;margin-bottom:14px}}
@media(max-height:760px){.heroclock{width:112px;height:112px;margin-bottom:12px}}
/* Below this the lead paragraph goes, and that frees enough room for the clock to
   stay on at its smallest. Dropping the clock here as well used to be necessary
   and is not any more: with the lead gone a 681px viewport has 108px of slack. */
@media(max-height:700px){
 .heroclock{width:84px;height:84px;margin-bottom:10px}
 .hero .lead{display:none}
}
/* and below this it really is the one element the hero can lose without losing
   meaning: the title, the thesis and the four numbers all carry information */
@media(max-height:600px){.heroclock{display:none}}
...
/* ──────────── credits: the stack, and the people ────────────
   Asked for at the end of the page. The logos carry two colours because the
   page flips from daylight to deep night and half of these marks are black or
   white; they are aria-hidden with the name as real text beside them, so a
   screen reader gets the word and not a shape. */
.credgrid{display:grid;grid-template-columns:1fr 1fr;gap:22px;align-items:stretch;margin-bottom:22px}
@media(max-width:860px){.credgrid{grid-template-columns:1fr}}
.credh{font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:var(--muted);
 font-weight:700;margin-bottom:18px}
...
/* the delay lives on the target state, which is what staggers the arrival;
   hover resets it so a late tile does not answer the mouse a second later */
.reveal.in .tool{opacity:1;transform:none;transition-delay:calc(var(--i,0) * 55ms)}
.tool:hover{transition-delay:0s;transform:translateY(-4px);
 border-color:var(--tc);box-shadow:0 14px 28px -16px var(--tc)}
html[data-mood="dark"] .tool:hover{border-color:var(--tcd);box-shadow:0 14px 28px -16px var(--tcd)}
.tool svg{width:28px;height:28px;display:block;fill:var(--tc)}
html[data-mood="dark"] .tool svg{fill:var(--tcd)}
.tool b{font-size:12px;font-weight:650;color:var(--ink);line-height:1.25}
.tool i{font-style:normal;font-size:10.5px;color:var(--muted);line-height:1.3}
@media(prefers-reduced-motion:reduce){
 .tool{opacity:1;transform:none;transition:none}
...
/* the averaging basis, stated wherever a ranking is read: without it a reader
   takes "paid work per day" for the length of a working day */
.tbase{display:block;margin-top:8px;font-size:12.5px;opacity:.82}
...
/* ──────────────── the closing image ────────────────
   A block of flats at night with one kitchen light still on: the unpaid shift
   without reducing anyone to a person with a broom. Measured luminance 4.4, so it
   sits inside the night band the page ends on instead of punching a hole in it.
   It comes after the closing question rather than inside the double-shift chapter,
   so the last thing on the page is an image and not a paragraph. */
.shiftimg{position:relative;border-radius:var(--r);overflow:hidden;
 box-shadow:0 24px 60px -30px rgba(0,0,0,.8)}
.shiftimg img{display:block;width:100%;height:auto}
.shiftimg::after{content:"";position:absolute;inset:auto 0 0 0;height:38%;
 background:linear-gradient(180deg,transparent,rgba(10,14,36,.72))}
.shiftcap{position:absolute;left:0;right:0;bottom:0;z-index:1;padding:20px 24px;
 font-size:13.5px;line-height:1.5;color:#e8ebff;text-shadow:0 2px 10px rgba(0,0,0,.7)}
.shiftcap b{color:#fff;font-weight:700}
@media(max-width:640px){.shiftcap{font-size:12.5px;padding:14px 16px}}

/* The final question used to live in the closing panel at display size and the
   picture underneath only carried a footnote, which put the crescendo above the
   last thing on the page. It moves onto the image, so the type has to grow and
   the scrim has to grow with it. The bottom of this artwork tops out at rgb(67,
   98, 143), so white sits at 15:1 even before the scrim deepens. */
.endimg{margin-top:34px}
.endimg::after{height:64%;background:linear-gradient(180deg,transparent,
 rgba(10,14,36,.5) 34%,rgba(10,14,36,.93))}
.endimg .shiftcap{padding:32px 36px 34px;text-align:center}
.endimg .ecl{display:block;font-size:14.5px;line-height:1.55;color:#cfd6ff}
.endimg .ecq{display:block;margin-top:12px;font-family:var(--display);
 font-size:clamp(25px,3.5vw,40px);font-weight:400;font-style:italic;line-height:1.16;
 letter-spacing:-.01em;color:#fff}
.endimg .ecq b{display:block;font-style:normal;font-weight:600}
/* On a narrow screen the artwork is only ~260px tall and the caption wants 147 of
   them, which leaves the picture as a strip behind text. Below 700px the caption
   drops out of the image and becomes a band underneath it, so both get their
   full size. */
@media(max-width:700px){
 .endimg::after{display:none}
 .endimg .shiftcap{position:static;padding:22px 20px 24px;
  background:linear-gradient(180deg,rgba(10,14,36,.94),rgba(10,14,36,.99))}
 .endimg .ecl{font-size:13px}
 .endimg .ecq{margin-top:10px}}
...
/* the panel now ends on the analysis; the question moved to the image below it */
...
<section class="reveal">
  ...
  <div class="twinwrap" id="twinWrap">
    <div class="panel st" style="--i:0"><div class="tcap">&#127968; Unpaid work / day <span>women do more</span></div><div class="chartbox twin"><canvas id="genderChart"></canvas></div></div>
    <div class="panel st" style="--i:7"><div class="tcap">&#128715; Leisure / day <span>men get more</span></div><div class="chartbox twin"><canvas id="leisureChart"></canvas></div></div>
  </div>
</section>
</div></div>
...
<section class="reveal" id="ch-change" data-nav="What can change">
  ...
  <!-- last thing on the page before the back matter: the question is asked in
  ...
  <figure class="shiftimg endimg">
    ...
    <figcaption class="shiftcap">
      <span class="ecl">Sooner or later that light goes out, the street goes quiet, and the whole day starts over from zero.</span>
      <span class="ecq">1,440 minutes, all over again. <b>What will you do with yours?</b></span>
    </figcaption>
  </figure>
</section>

<section class="reveal" id="ch-made">
  <div class="sintro">
  ...
  <div class="credgrid">
    <div class="ai">
      <h3 class="credh">How we used AI</h3>
      <ul>
        ...
        <li>Illustration: the eight skies, the five portraits and the topic art are generated images</li>
        <li>Narrative drafting and accessibility review</li>
      </ul>
    </div>
    <div class="ai">
      <h3 class="credh">Made by</h3>
      __PEOPLE__
    </div>
  </div>
  <div class="ai">
    <h3 class="credh">Tools &amp; technologies</h3>
    <p class="crednote">No chart wizard and no front-end framework: <b>Python</b> reads the CSVs and
      writes the markup, <b>Chart.js</b> draws the rings and bars, <b>Three.js</b> (through globe.gl)
      spins the globe, <b>FFmpeg</b> compresses the video and stills, and <b>GitHub Pages</b> serves
      the one file that comes out.</p>
    __TOOLROW__
  </div>
</section>
    ...
    <div>
      ...
      <ul>
        ...
        <li>The five categories sum to 24 hours, so every chart here is a share of one real day.
          ...
          per person, and so cannot be compared with the daily figures.</li>
        ...
        <li><b>Reference years differ by country</b>, each being the most recent available, so this
...
/* Every section starts at opacity 0 and waits on that observer, so if it ever
   fails the page reads as blank. Cheap insurance: after 10s, reveal anything
   still hidden. Nobody should lose the whole story to one API. */
setTimeout(()=>document.querySelectorAll('.reveal:not(.in)').forEach(el=>el.classList.add('in')),10000);
...
const TOPICS=[
  ...
  {k:'paw',img:'paid-work',col:COL[1],unit:'h',
   name:'Paid work',
   what:'Time on the job or studying, averaged over everyone and every day of the week.',
...
/* Two beats, not one. The copy says the penalty is double, but both charts used
   to arrive in the same instant, so the second half of the argument landed for
   free. The unpaid-work chart comes in first; leisure follows after a pause, and
   the bars are only built once the section is on screen so the growth is seen
   rather than played to an empty room. */
const twin=document.getElementById('twinWrap');
let twinBuilt=false;
function buildTwin(){
  if(twinBuilt)return;twinBuilt=true;
  ...
  // the leisure chart waits out the first beat before it starts drawing
  setTimeout(()=>{
  ...
  retintAll(mood);
  void twin.offsetHeight;twin.classList.add('go');
}
/* Three paths to the same call, because the charts must exist even if the timing
   trick does not. buildTwin is idempotent, so whichever fires first wins:
   the observer for the nice moment, a plain rect check for browsers where it
   misbehaves, and a timer as the guarantee. Losing the animation is a shame;
   losing the climax's two charts is not acceptable. */
const twinInView=()=>{
  const r=twin.getBoundingClientRect();
  return r.top<window.innerHeight*0.9 && r.bottom>0;
};
new IntersectionObserver((es,ob)=>es.forEach(e=>{
  if(e.isIntersecting){buildTwin();ob.disconnect();}
}),{threshold:.15}).observe(twin);
window.addEventListener('scroll',()=>{if(twinInView())buildTwin();},{passive:true});
if(twinInView())buildTwin();
setTimeout(buildTwin,8000);

// the whole point of these two charts is the numbers, so they exist as text too
const gTop=g[0];
// (safe before the charts are built: these only touch the canvas and its panel)
...
(function closing(){
  ...
  // the question that used to sit here now closes the page over the image below
})();
...
<!-- A pagina e publicada em dois lugares: a vitrine da organizacao e o fork pessoal
     que serviu de rascunho. O canonical aponta os dois para o endereco oficial, para
     que buscador e preview de link nunca mostrem o rascunho. -->
<link rel="canonical" href="https://the-24-hour-human.github.io/">
<meta property="og:title" content="The 24-Hour Human">
<meta property="og:description" content="The fairest thing the world hands out, and the most unequal thing we do with it. 1,440 minutes a day, across 35 countries.">
<meta property="og:url" content="https://the-24-hour-human.github.io/">
<meta property="og:type" content="website">
<!-- o card e um retrato do proprio hero, gerado com o titulo e os contadores no
     estado final. JPEG de proposito: WebP nao e lido por todo gerador de preview -->
<meta property="og:image" content="https://the-24-hour-human.github.io/assets/img/og-card.jpg">
<meta property="og:image:width" content="1200">
<meta property="og:image:height" content="630">
<meta property="og:image:alt" content="The 24-Hour Human, over a clock and the numbers 1,440 minutes, 35 countries, 5 continents.">
...
FINAL = HTML.replace("__PAW_NOTE__", PAW_NOTE).replace("__DEF_NOTE__", DEF_NOTE).replace("__TOOLROW__", TOOLROW_HTML).replace("__PEOPLE__", PEOPLE_HTML).replace("__GRAIN__", GRAIN).replace("__TICKS__", CLOCK_TICKS).replace("__HERO_LEI_GAP__", HERO_LEI_GAP).replace("__DATA__", DATA)
check_assets(FINAL)
