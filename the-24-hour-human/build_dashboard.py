...
.pdetail.go .st{opacity:1;transform:none}
@media(prefers-reduced-motion:reduce){
...
.qclock{position:relative;width:158px;height:158px;flex:0 0 auto}
...
.mpclock{position:relative;width:176px;height:176px;margin:14px 0}
...
/* ──────────────── who works the most: flag bubbles ──────────────── */
...
.predclocks{display:flex;align-items:center;justify-content:center;gap:30px;flex-wrap:wrap}
.clockbox2{position:relative;width:208px;height:208px}
.clab{text-align:center;font-size:11px;letter-spacing:.18em;text-transform:uppercase;
 color:var(--muted);font-weight:700;margin-bottom:10px}
.predclocks .arrow{font-size:32px;color:var(--accent)}
...
<section class="reveal" id="ch-quiz" data-nav="Your match">
  <div class="shead"><span class="chapno">02</span><span class="sh">Now your turn</span><span class="tchip">09:00</span></div>
...
<section class="reveal" id="ch-globe" data-nav="The globe">
  <div class="shead"><span class="chapno">03</span><span class="sh">Explore all 35</span><span class="tchip">12:00 &middot; Midday</span></div>
...
<section class="reveal" id="ch-work" data-nav="Who works most">
  <div class="shead"><span class="chapno">04</span><span class="sh">The first surprise</span><span class="tchip">15:00 &middot; Afternoon</span></div>
  <h2>Who works <em>the most?</em></h2>
...
/* one shared recipe for every 24h ring */
const ring=(data,cut='68%')=>({type:'doughnut',
  data:{labels:CATS,datasets:[{data,backgroundColor:COL,borderWidth:0,spacing:2,borderRadius:5,hoverOffset:9}]},
  options:{cutout:cut,plugins:{legend:{display:false},
...
/* ── the sky: scroll is the passage of a day ──────────────────────────────────
   ...
   image has fully arrived when its chapter is on screen:
     0.00 morning    hero + "five lives"      (07:00)
     0.30 midday     "your match" + globe     (09:00 / 12:00)
     0.44 afternoon  "who works most"         (15:00)
     0.53 golden     "the double shift"       (20:00)
     0.65 sunset     "work vs. free time"     (21:30)
     0.76 dusk       "working until when"     (22:30)
     0.86 night      "the day of 2050"        (23:00)
     1.00 midnight   "the takeaway"           (23:59, the day ends)
   The story closes on deep night, matching its own closing line ("when your day
   ends..."). One single ink flip, to light at p=0.705, between two chapters.
...
const SKY=[
 ...
 {p:0.30,img:'02-midday',   dark:false,c:['#a6d7df','#9fe2ee','#79e2fb']},
 {p:0.44,img:'03-afternoon',dark:false,c:['#b8c9c3','#d0d5bc','#eecf91']},
 {p:0.53,img:'04-golden',   dark:false,c:['#efaf7d','#dca686','#e8b578']},
 {p:0.65,img:'05-sunset',   dark:false,c:['#ac9d8d','#e6a26e','#e99a75']},
 {p:0.76,img:'06-dusk',     dark:true, c:['#4a4373','#75699c','#7b87b5']},
...
D.personas.forEach((p,i)=>{
  ...
  new Chart(document.getElementById('clk'+i),ring(p.clock,'70%'));
});
...
document.getElementById('qbtn').addEventListener('click',()=>{
  ...
  if(qc)qc.destroy();
  qc=new Chart(document.getElementById('qclock'),ring([best.pca,best.paw,best.upw,best.lei,best.oth],'62%'));
});

/* ── who works the most: flag bubbles sized and ringed by hours/year ── */
...
document.getElementById('workBubbles').innerHTML=top10.map(c=>{
  const t=(c.wh-whLo)/(whHi-whLo),dia=Math.round(66+t*94);
  return `<div class="bub" style="--ringc:${heat(t)}">
...
function drawClock(id,data,ref){
  ...
  ref.c=new Chart(document.getElementById(id),ring(data,'62%'));
}
function renderPred(){
...
function selectCountry(c){
  ...
  mapClock=new Chart(document.getElementById('mpClock'),ring([c.pca,c.paw,c.upw,c.lei,c.oth],'62%'));
}
